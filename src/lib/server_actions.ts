...
export async function createReviewSQL(
  ...
  try {
    ...
    const location = await prisma.$transaction(async (tx) => {
      const location = data.id
        ? await getLocation(data.id, tx)
        : await createLocation(reviewData?.mapState!, data.location, tx);
      if (!location) {
    ...
    return {
      ...
      errors: null,
      action: data.id
        ? `Review added to location: ${location.name}.`
        : `New location ${location.name} added with review.`,
    };
  } catch (error: any) {
    console.log("error creating review", error);
    return {
...
export async function deleteReviewAction(
  ...
  try {
    ...
    reviews = await getReviews(reviewData.locationId);

    if (reviews?.length === 0) {
      console.log("deleted review and location");
    ...
    // needs to be like this otherwise it wont redirect from the deletePopup component
    if (reviews.length === 0) {
      redirect("/");
    }
  }
