...
export default function ReviewImageCreator({ setImageData, imgString }: Props) {
    ...
    function takePicture() {
        ...
        if (isMobile) {
            // MOBILE
            previewCanvas.current!.setAttribute(
                "width",
        ...
        if (isMobile) {
            ...
            context!.drawImage(
                video.current!,
                0,
                video.current!.videoHeight / 2 - video.current!.videoWidth / 2,
            ...
            context!.drawImage(
                video.current!,
                Math.abs(
                    video.current!.videoWidth / 2 - video.current!.videoHeight / 2,
                ),
                0, // from x and y // video.videowidth
                video.current!.videoHeight,
                video.current!.videoHeight, // take square
                0,
                0, // draw to canvas
                previewCanvas.current!.width,
                previewCanvas.current!.height, // size that is drawn
            );
        }
        ...
        <div>
            <CameraPortal>
                ...
                >
                    ...
                    <div className={styles.captureImage}>
                        ...
                        <div className={clsx(styles.optionButtons, confirmingPicture && styles.hide)}>
                            <button className={styles.button} onClick={takePicture}>
                                Take picture
                            </button>
                            <button className={styles.button} onClick={cancelTakingPicture}>
                                Cancel
                            </button>
                        </div>
                    </div>
                    <div
                        className={clsx(
                            confirmingPicture && styles.show,
                            styles.confirmingOutput,
                        )}
                    >
                        <canvas ref={previewCanvas} className={styles.previewCanvas} />
                        <div className={styles.optionButtons}>
                            <button className={styles.button} onClick={savePicture}>
                                Save
                            </button>
                            <button className={styles.button} onClick={retakePicture}>
                                Retake
                            </button>
                            <button className={styles.button} onClick={cancelTakingPicture}>
                                Cancel
            ...
            <div className={styles.output}>
                <Image
                    ref={finalPhoto}
                    src={imageUrl}
                    alt="the saved photo to be uploaded"
                    height={100}
                    width={100}
                />
            </div>
            <button
                ...
                ref={startButton}
                onClick={startUp}
                className={clsx(styles.button, styles.takePhoto)}
            >
                {pictureSaved ? "Retake" : "Take"} photo
            </button>
        </div>
    );
}
